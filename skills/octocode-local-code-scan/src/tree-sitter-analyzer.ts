...
function collectTreeSitterMetrics(node: SyntaxNode, _sourceText: string): TreeSitterMetrics {
  ...
  const visit = (n: SyntaxNode, branchDepth: number, loopDepth: number): void => {
    metrics.statements += 1;

    if (
      ['if_statement', 'while_statement', 'do_statement', 'for_statement', 'for_in_statement', 'for_of_statement',
...
function inferTreeSitterFunctionName(node: SyntaxNode, _text: string): string {
  const identifier = node.namedChildren.find((child) =>
    ['identifier', 'property_identifier', 'type_identifier'].includes(child.type)
  );
  ...
  while (parent) {
    if (parent.type === 'variable_declarator') {
      const id = parent.namedChildren.find((child) =>
        ['identifier', 'property_identifier', 'array_pattern', 'object_pattern', 'shorthand_property_identifier_pattern'].includes(child.type)
      );
    ...
    if (parent.type === 'pair') {
      const key = parent.namedChildren.find((child) =>
        ['identifier', 'string', 'shorthand_property_identifier_pattern', 'property_identifier'].includes(child.type)
      );
...
function countControlFlowBodyStatements(node: SyntaxNode): number {
  const body = node.namedChildren.find((child) =>
    child.type === 'statement_block' || child.type === 'switch_body'
  );
  if (body) return body.namedChildren.length;
  return node.namedChildren.length;
}

function makeLocationFromTree(node: SyntaxNode, repoRoot: string, filePath: string): Location {
  return {
...
export function analyzeTreeSitterFile(filePath: string, sourceText: string, options: AnalysisOptions, packageName: string, maps: FlowMaps | null): TreeSitterFileEntry | null {
...
export async function resolveTreeSitter(): Promise<TreeSitterRuntime> {
  ...
  try {
    ...
    const typescriptMod: Record<string, unknown> = await import('tree-sitter-typescript');

    const ParserClass = parserMod.default || parserMod;

    const tsLang = (typescriptMod as Record<string, unknown>).typescript || ((typescriptMod as Record<string, unknown>).default as Record<string, unknown>)?.typescript;
    const tsxLang = (typescriptMod as Record<string, unknown>).tsx || ((typescriptMod as Record<string, unknown>).default as Record<string, unknown>)?.tsx;
    ...
    parserTsx.setLanguage((tsxLang || tsLang) as Parameters<Parser['setLanguage']>[0]);
