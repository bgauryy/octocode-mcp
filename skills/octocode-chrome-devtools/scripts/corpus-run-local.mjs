...
const candidates = [
  ...(explicit ? [join(explicit, 'scripts', 'corpus-run.mjs')] : []),
  ...(!explicit ? [join(here, '..', '..', 'octocode-scraping', 'scripts', 'corpus-run.mjs')] : []),
];
...
if (!target) {
  console.error(JSON.stringify({
    ...
    error: 'The optional octocode-scraping skill is required for corpus queries. Install it beside octocode-chrome-devtools or pass --scraping-skill-dir <dir>.',
  }));
  process.exit(1);
}
