...
Current remote surfaces are `ghSearch` for GitHub code, repositories, or trees; `ghGetFileContent` for known paths; `ghSearchHistory` for PR, issue, or commit metadata; `ghGetHistoryItem` for a known item; `ghCloneRepo` for an opt-in local checkout; and `npmSearch` for package metadata. Local surfaces are `localSearch` for lexical text/regex, `astSearch` for `match`, `files`, `tree`, `symbols`, or `topology`, `localGetFileContent` for exact or explicitly minified reads, and `lspSearch` for semantic operations. Local search takes `path` and `searchText`; LSP takes a real URI plus an exact position or name/line anchor. Read source before an anchored LSP call and follow returned continuations unchanged.
