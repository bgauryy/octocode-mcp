...
npx -y octocode-cli local-file --path ./src/auth/handler.ts --match-string "handleAuth" --match-string-context-lines 10
```
...
npx -y octocode-cli lsp-call-hierarchy --uri ./src/auth.ts --symbol-name "validateToken" --line-hint 42 --direction outgoing

# 6. Read implementation LAST
npx -y octocode-cli local-file --path ./src/auth.ts --match-string "validateToken" --match-string-context-lines 20
```
...
npx -y octocode-cli local-file --path ./src/found-file.ts --match-string "TODO"
```
...
npx -y octocode-cli local-file --path ./src/large-file.ts --match-string "exportFunction" --match-string-context-lines 10
...
npx -y octocode-cli local-file --path ./node_modules/express/lib/express.js --match-string "createServer"
```
...
npx -y octocode-cli search-packages --name "state-management" --ecosystem npm --search-limit 5
...
npx -y octocode-cli search-repos --keywords-to-search "react,testing,library" --sort stars --limit 5

# 2. Explore structure
npx -y octocode-cli tree --owner testing-library --repo react-testing-library --path src --depth 2

# 3. Search for patterns
npx -y octocode-cli search-code --keywords-to-search "render,screen" --owner testing-library --repo react-testing-library --extension ts
...
npx -y octocode-cli search-code --keywords-to-search "Router,handle" --owner expressjs --repo express --extension js
...
npx -y octocode-cli search-prs --owner expressjs --repo express --pr-number 5678 --with-commits --type fullContent
...
npx -y octocode-cli search-code --keywords-to-search "Router,use" --owner expressjs --repo express --extension js

# 4. Read the relevant source
npx -y octocode-cli get-file --owner expressjs --repo express --path lib/router/index.js --match-string "use"
```
...
npx -y octocode-cli search-repos --keywords-to-search "authentication,jwt,middleware" --sort stars --limit 5

# 2. EXPLORE — Understand structure
npx -y octocode-cli tree --owner auth0 --repo express-jwt --depth 2

# 3. SEARCH — Find patterns
npx -y octocode-cli search-code --keywords-to-search "verify,middleware" --owner auth0 --repo express-jwt --extension ts

# 4. READ — Get implementation (LAST)
npx -y octocode-cli get-file --owner auth0 --repo express-jwt --path src/index.ts --match-string "verify"
