...
export function detectLowAssertionDensity(fileSummaries: FileEntry[]): FindingDraft[] {
...
export function detectTestNoAssertion(fileSummaries: FileEntry[]): FindingDraft[] {
  const findings: FindingDraft[] = [];
  for (const entry of fileSummaries) {
    if (!isTestFile(entry.file) || !entry.testProfile) continue;
...
export function detectExcessiveMocking(fileSummaries: FileEntry[], mockThreshold: number = 10): FindingDraft[] {
  const findings: FindingDraft[] = [];
  for (const entry of fileSummaries) {
    if (!isTestFile(entry.file) || !entry.testProfile) continue;
...
export function detectSharedMutableState(fileSummaries: FileEntry[]): FindingDraft[] {
  const findings: FindingDraft[] = [];
  for (const entry of fileSummaries) {
    if (!isTestFile(entry.file) || !entry.testProfile) continue;
...
export function detectMissingTestCleanup(fileSummaries: FileEntry[]): FindingDraft[] {
  const findings: FindingDraft[] = [];
  for (const entry of fileSummaries) {
    if (!isTestFile(entry.file) || !entry.testProfile) continue;
...
export function detectFakeTimersWithoutRestore(fileSummaries: FileEntry[]): FindingDraft[] {
  const findings: FindingDraft[] = [];
  for (const entry of fileSummaries) {
    if (!isTestFile(entry.file) || !entry.testProfile) continue;
    const fakeTimerCalls = entry.testProfile.timerControls.filter((call) =>
      call.kind === 'jest.useFakeTimers' || call.kind === 'vi.useFakeTimers'
    );
    if (fakeTimerCalls.length === 0) continue;

    const hasRestore = entry.testProfile.timerControls.some((call) =>
...
export function detectMissingMockRestoration(fileSummaries: FileEntry[]): FindingDraft[] {
  const findings: FindingDraft[] = [];
  for (const entry of fileSummaries) {
    if (!isTestFile(entry.file) || !entry.testProfile) continue;
