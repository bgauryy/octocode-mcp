...
import { buildAdvancedGraphFindings, computeChokepoints, computeGraphAnalytics, computePackageGraphSummary, computeSccClusters, packageKeyForFile } from './graph-analytics.js';

import type { DependencyState, DependencySummary, FileCriticality, FileEntry } from './types.js';
...
function minimalSummary(overrides: Partial<DependencySummary> = {}): DependencySummary {
  return {
    totalModules: 0,
    totalEdges: 0,
...
function makeFile(file: string, topLevelEffects: FileEntry['topLevelEffects'] = []): FileEntry {
  return {
    package: 'test',
    file,
...
describe('graph analytics', () => {
  it('groups files into SCC clusters', () => {
    ...
    for (const file of state.files) criticality.set(file, { file, complexityRisk: 1, highComplexityFunctions: 0, functionCount: 1, flows: 0, score: 5 });
  ...
  it('computes package hotspots and chokepoints', () => {
    ...
    const criticality = new Map<string, FileCriticality>();
    for (const file of state.files) criticality.set(file, { file, complexityRisk: 1, highComplexityFunctions: 0, functionCount: 1, flows: 0, score: 12 });
  ...
  it('builds advanced graph findings for cycle clusters and startup hubs', () => {
    ...
    const criticality = new Map<string, FileCriticality>();
    for (const file of state.files) criticality.set(file, { file, complexityRisk: 1, highComplexityFunctions: 0, functionCount: 1, flows: 0, score: 20 });
    const analytics = computeGraphAnalytics(state, minimalSummary(), criticality);
...
describe('computeChokepoints', () => {
  it('detects articulation points', () => {
    ...
    addEdge(state, 'src/b.ts', 'src/c.ts');
    const criticality = new Map<string, FileCriticality>();
    for (const f of state.files) criticality.set(f, { file: f, complexityRisk: 1, highComplexityFunctions: 0, functionCount: 1, flows: 0, score: 5 });
    const chokepoints = computeChokepoints(state, minimalSummary(), criticality, []);
  ...
  it('filters out nodes with score=0 and no reasons', () => {
    ...
    const criticality = new Map<string, FileCriticality>();
    criticality.set('src/isolated.ts', { file: 'src/isolated.ts', complexityRisk: 0, highComplexityFunctions: 0, functionCount: 0, flows: 0, score: 0 });
    const chokepoints = computeChokepoints(state, minimalSummary(), criticality, []);
...
describe('buildAdvancedGraphFindings - broker-module', () => {
  it('generates broker-module findings for high fan-in/fan-out chokepoints', () => {
    ...
    const criticality = new Map<string, FileCriticality>();
    for (const file of state.files) criticality.set(file, { file, complexityRisk: 1, highComplexityFunctions: 0, functionCount: 1, flows: 0, score: 30 });
    const analytics = computeGraphAnalytics(state, minimalSummary(), criticality);
    const findings = buildAdvancedGraphFindings(analytics, state, []);
    expect(findings.some(f => f.category === 'broker-module' && f.file === hub)).toBe(true);
  });
});
...
describe('buildAdvancedGraphFindings - bridge-module', () => {
  it('generates bridge-module findings for articulation points', () => {
    ...
    const analytics = computeGraphAnalytics(state, minimalSummary(), criticality);
    const findings = buildAdvancedGraphFindings(analytics, state, []);
    expect(findings.some(f => f.category === 'bridge-module' && f.file === 'src/bridge.ts')).toBe(true);
  });
});
...
describe('buildAdvancedGraphFindings - import line resolution', () => {
  it('uses importedSymbolsByFile for line numbers when available', () => {
    ...
    state.importedSymbolsByFile.set('src/a.ts', [
      { sourceModule: './b', resolvedModule: 'src/b.ts', importedName: 'x', localName: 'x', isTypeOnly: false, lineStart: 5, lineEnd: 7 },
    ]);
    const criticality = new Map<string, FileCriticality>();
    for (const f of state.files) criticality.set(f, { file: f, complexityRisk: 1, highComplexityFunctions: 0, functionCount: 1, flows: 0, score: 25 });
    const analytics = computeGraphAnalytics(state, minimalSummary(), criticality);
    const findings = buildAdvancedGraphFindings(analytics, state, []);
...
describe('buildAdvancedGraphFindings - package-boundary-chatter', () => {
  it('generates package-boundary-chatter findings when hotspot edges >= 4', () => {
    ...
    const analytics = computeGraphAnalytics(state, minimalSummary(), criticality);
    const findings = buildAdvancedGraphFindings(analytics, state, []);
    expect(findings.some(f => f.category === 'package-boundary-chatter')).toBe(true);
  });
});
