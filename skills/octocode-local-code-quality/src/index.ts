...
import type {
  AnalysisOptions, DependencyState, DependencySummary,
  DuplicateGroup, FileCriticality,
  FileEntry, Finding, FlowMapEntry,
  RedundantFlowGroup,
} from './types.js';
...
if (isDirectRun) {
  import('./pipeline.js').then((m) => m.main()).catch((error: unknown) => {
    console.error(error);
    process.exit(1);
  });
}
