...
const toSecurityFinding = (draft, ruleId, confidence, evidence) => ({
    ...draft,
    ruleId,
    confidence,
    evidence,
});
export function detectHardcodedSecrets(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (isTestFile(entry.file))
            continue;
        const secrets = (entry.suspiciousStrings || []).filter(s => s.kind === 'hardcoded-secret' &&
...
export function detectEvalUsage(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (isTestFile(entry.file))
            continue;
...
export function detectUnsafeHtml(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (isTestFile(entry.file))
            continue;
...
export function detectSqlInjectionRisk(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (isTestFile(entry.file))
            continue;
...
export function detectUnsafeRegex(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (isTestFile(entry.file))
            continue;
...
export function detectPrototypePollutionRisk(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (isTestFile(entry.file))
            continue;
        if (!entry.prototypePollutionSites ||
        ...
        for (const site of entry.prototypePollutionSites) {
            ...
            findings.push(toSecurityFinding({
                ...
                lspHints: [
                    {
                        tool: 'lspCallHierarchy',
                        symbolName: site.kind === 'computed-property-write'
                            ? 'bracket-assignment'
                            : site.detail.split('(')[0],
...
export function detectUnvalidatedInputSink(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (isTestFile(entry.file))
            continue;
        for (const src of entry.inputSources || []) {
            if (!src.hasSinkInBody || src.hasValidation)
                continue;
...
export function detectInputPassthroughRisk(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (isTestFile(entry.file))
            continue;
        for (const src of entry.inputSources || []) {
            if (src.callsWithInputArgs.length === 0 || src.hasValidation)
                continue;
            if (src.hasSinkInBody)
                continue;
            if (src.paramConfidence === 'low')
                continue;
...
export function detectPathTraversalRisk(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (isTestFile(entry.file))
            continue;
        for (const src of entry.inputSources || []) {
            ...
            if (fsReadSinks.length === 0)
                continue;
            if (src.paramConfidence === 'low')
                continue;
            const hasValidation = src.hasValidation;
            const severity = hasValidation ? 'medium' : 'high';
...
export function detectCommandInjectionRisk(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (isTestFile(entry.file))
            continue;
        for (const src of entry.inputSources || []) {
            ...
            if (src.paramConfidence === 'low')
                continue;
            const execCallees = src.callsWithInputArgs.filter(c => /\.exec\b|^exec$|^execSync$|child_process\.exec/.test(c.callee));
            const spawnCallees = src.callsWithInputArgs.filter(c => /\.spawn\b|^spawn$|^spawnSync$|child_process\.spawn/.test(c.callee));
            if (execCallees.length > 0) {
                const severity = src.paramConfidence === 'high' ? 'critical' : 'high';
                findings.push(toSecurityFinding({
                    severity,
