...
export function detectDuplicateFunctionBodies(duplicates) {
    ...
    for (const group of duplicates) {
        ...
        const severity = group.occurrences >= 6
            ? 'high'
            : group.occurrences >= 3
                ? 'medium'
                : 'low';
        if (!canAddFinding(findings))
            break;
        findings.push({
            ...sample,
            severity,
...
export function detectFunctionOptimization(fileSummaries, criticalComplexityThreshold) {
    ...
    for (const fileEntry of fileSummaries) {
        for (const fn of fileEntry.functions) {
            ...
            if (alerts.length === 0)
                continue;
            const isHigh = fn.complexity >= criticalComplexityThreshold ||
                fn.maxBranchDepth >= 7 ||
                fn.maxLoopDepth >= 4;
            findings.push({
                ...fn,
...
export function detectSwitchNoDefault(fileSummaries) {
    ...
    for (const entry of fileSummaries) {
        if (isTestFile(entry.file))
            continue;
        if (!entry.switchesWithoutDefault ||
...
export function detectLowMaintainability(fileSummaries, threshold = 20) {
    ...
    for (const entry of fileSummaries) {
        if (isTestFile(entry.file))
            continue;
        for (const fn of entry.functions) {
            if (fn.maintainabilityIndex == null ||
        ...
        for (let i = 0; i < bucket.length; i++) {
            for (let j = i + 1; j < bucket.length; j++) {
                ...
                if (a.file === b.file && a.lineStart === b.lineStart)
                    continue;
                const stmtRatio = Math.min(a.statementCount, b.statementCount) /
