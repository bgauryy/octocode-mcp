...
// ─── Architecture Risk ──────────────────────────────────────────────────────
export function detectSemanticDeadExports(profiles) {
    const findings = [];
...
export function detectOverAbstraction(ctx, profiles) {
    const findings = [];
    ...
    for (const profile of profiles) {
        ...
        const visit = (node) => {
            if (ts.isInterfaceDeclaration(node) && node.name) {
                ...
                const impls = ctx.service.getImplementationAtPosition(path.resolve(ctx.root, profile.file), node.name.getStart(sourceFile));
...
export function detectConcreteDependency(profiles) {
    const findings = [];
    for (const profile of profiles) {
...
export function detectCircularTypeDependency(ctx, profiles) {
    const findings = [];
    ...
    for (const profile of profiles) {
        ...
        for (const [typeName, refs] of fileTypeRefs) {
            if (!typeGraph.has(typeName))
                typeGraph.set(typeName, new Set());
            for (const ref of refs)
                typeGraph.get(typeName).add(ref);
        }
    }
    const visited = new Set();
    const inStack = new Set();
    const reportedCycles = new Set();
    const dfs = (node, stackPath) => {
        if (inStack.has(node)) {
...
// ─── Code Quality ───────────────────────────────────────────────────────────
export function detectUnusedParameters(profiles) {
    const findings = [];
    for (const profile of profiles) {
...
export function detectTypeHierarchyDepth(profiles, threshold = 4) {
    const findings = [];
    for (const profile of profiles) {
...
export function detectDeepOverrideChain(profiles, threshold = 3) {
    const findings = [];
    for (const profile of profiles) {
...
export function detectInterfaceCompliance(profiles) {
    const findings = [];
    for (const profile of profiles) {
...
// ─── Dead Code & Hygiene ────────────────────────────────────────────────────
export function detectUnusedImports(profiles) {
    const findings = [];
    for (const profile of profiles) {
...
export function detectOrphanImplementation(ctx, profiles) {
    const findings = [];
    for (const profile of profiles) {
        const sourceFile = ctx.program.getSourceFile(path.resolve(ctx.root, profile.file));
        if (!sourceFile)
        ...
        const visit = (node) => {
            if (ts.isClassDeclaration(node) && node.name) {
                ...
                const refs = ctx.service.findReferences(path.resolve(ctx.root, profile.file), node.name.getStart(sourceFile));
                let externalUsage = 0;
                if (refs) {
...
// ─── Shotgun Surgery ────────────────────────────────────────────────────────
export function detectShotgunSurgery(profiles, threshold = 8) {
    const findings = [];
    for (const profile of profiles) {
        for (const [name, info] of profile.referenceCountByExport) {
...
export function detectMoveToCaller(profiles) {
    const findings = [];
    for (const profile of profiles) {
        for (const [name, info] of profile.referenceCountByExport) {
...
// ─── Leaky Abstraction ──────────────────────────────────────────────────────
export function detectLeakyAbstraction(profiles) {
    const findings = [];
    for (const profile of profiles) {
...
// ─── Narrowable Type ────────────────────────────────────────────────────────
export function detectNarrowableType(profiles) {
    const findings = [];
    for (const profile of profiles) {
...
// ─── Orchestrator ───────────────────────────────────────────────────────────
export function runSemanticDetectors(ctx, profiles, options = {}) {
    const all = [];
