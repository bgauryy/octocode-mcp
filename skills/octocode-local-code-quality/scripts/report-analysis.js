...
export function enrichFindings(findings, fileEntries, hotFiles, graphAnalytics) {
    ...
    return findings.map((finding) => {
        ...
        for (const category of relatedByFile.get(finding.file) || new Set()) {
...
export function computeReportAnalysisSummary(findings, fileEntries, hotFiles, graphAnalytics) {
    ...
    if (graphAnalytics?.sccClusters.length) {
        ...
        graphSignals.push(makeSignal('cycle-cluster', 'graph', 'Cycle cluster', `${cluster.id} links ${cluster.nodeCount} files into a single strongly connected group.`, cluster.nodeCount >= 5 ? 'high' : 'medium', cluster.nodeCount * 6 + cluster.edgeCount, cluster.files, ['dependency-cycle', 'cycle-cluster'], { clusterId: cluster.id, nodeCount: cluster.nodeCount, hubFiles: cluster.hubFiles }));
    }
    ...
    for (const entry of fileEntries) {
        ...
        if ((entry.effectProfile?.totalEffects || 0) > 0 && categories.has('import-side-effect-risk')) {
            astSignals.push(makeSignal('hidden-initialization', 'ast', 'Hidden initialization logic', `${entry.file} performs import-time work that matches the reported side-effect risk.`, 'medium', 75, [entry.file], ['import-side-effect-risk'], { totalEffects: entry.effectProfile?.totalEffects, highestRisk: entry.effectProfile?.highestRisk }));
        }
    ...
    if (strongestGraphSignal && strongestAstSignal) {
        ...
        combinedInterpretation = makeSignal('combined-interpretation', 'hybrid', 'Combined interpretation', sharedFile
            ...
            : `${strongestGraphSignal.title} and ${strongestAstSignal.title} both appear in this scan, so use a hybrid investigation instead of a single-lens conclusion.`, confidence, Math.round((strongestGraphSignal.score + strongestAstSignal.score) / 2), sharedFile ? [sharedFile] : [...new Set([...strongestGraphSignal.files, ...strongestAstSignal.files])].slice(0, 4), [...new Set([...strongestGraphSignal.categories, ...strongestAstSignal.categories])], {
