...
export function collectSearchFiles(root, opts) {
    const files = [];
    const walk = (dir) => {
        let entries;
        try {
...
function nodeToMatch(node, file, pattern) {
    ...
    if (pattern) {
        ...
        if (Object.keys(vars).length > 0)
            result.metaVariables = vars;
    }
    return result;
}
export function searchFile(filePath, source, matcher, patternStr, limit) {
...
export function runSearch(files, opts, root) {
    let matcher;
    let queryLabel;
    let queryType;
    let patternStr = null;
...
export function formatTextOutput(result, opts, _root) {
    ...
    for (const m of result.matches) {
        ...
        const truncatedText = m.text.length > 200 ? m.text.slice(0, 200) + '…' : m.text;
