...
function buildDependencySummary(dependencyState, fileCriticalityByPath, options) {
    ...
    const criticalNodes = allFiles
        .map((file) => ({ ...dependencyProfileToRecord(file, dependencyState), ...(fileCriticalityByPath.get(file) || {}) }))
    ...
    return {
        ...
        criticalModules: criticalNodes.slice(0, 20),
...
export function computeDependencyCycles(dependencyState) {
    ...
    const canonicalizeCycle = (cyclePath) => {
...
export function buildIssueCatalog(duplicates, controlDuplicates, fileSummaries, dependencySummary, dependencyState, options, pkgJsonDeps = {}, pkgJsonDevDeps = {}, fileCriticalityByPath = new Map(), semanticFindings = [], flowMap = new Map(), additionalFindings = []) {
    ...
    for (const f of detectLowCohesion(dependencyState))
        addFinding(f);
    for (const f of detectInferredLayerViolations(dependencyState))
        addFinding(f);
    ...
    for (const f of detectSwitchNoDefault(fileSummaries))
        addFinding(f);
    for (const f of detectHighCyclomaticDensity(fileSummaries, options.cyclomaticDensityThreshold))
        addFinding(f);
    for (const f of detectUnsafeAny(fileSummaries, options.anyThreshold))
        addFinding(f);
    for (const f of detectMagicNumbers(fileSummaries, options.magicNumberThreshold))
        addFinding(f);
...
function generateMermaidGraph(dependencyState, dependencySummary, _fileCriticalityByPath) {
    const lines = [];
    lines.push('# Dependency Graph\n');
    lines.push('## Module Dependency Map\n');
    lines.push('```mermaid');
    lines.push('graph LR');
    const criticalFiles = new Set((dependencySummary.criticalModules || []).map((m) => m.file));
    const cycleFiles = new Set();
    for (const cycle of dependencySummary.cycles || []) {
        for (const f of cycle.path)
            cycleFiles.add(f);
    }
    const shorten = (filePath) => {
