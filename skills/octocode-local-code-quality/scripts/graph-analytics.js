export function packageKeyForFile(file) {
...
export function computeSccClusters(dependencyState) {
    ...
    const strongConnect = (node) => {
        ...
        for (const next of outgoing) {
            if (!dependencyState.files.has(next))
                continue;
        ...
        for (const file of component) {
            ...
            for (const incoming of dependencyState.incoming.get(file) || new Set()) {
                if (!componentSet.has(incoming))
                    entryEdges += 1;
            }
        }
...
export function computePackageGraphSummary(dependencyState) {
    const nodeMap = new Map();
    const edgeMap = new Map();
    for (const file of dependencyState.files) {
        const pkg = packageKeyForFile(file);
        if (!nodeMap.has(pkg))
            nodeMap.set(pkg, { package: pkg, inbound: 0, outbound: 0, internalFiles: 0 });
    ...
    for (const [from, outgoing] of dependencyState.outgoing.entries()) {
        ...
        for (const to of outgoing) {
            if (!dependencyState.files.has(to))
                continue;
    ...
    const hotspots = [...edgeMap.entries()]
        .map(([key, edges]) => {
...
function computeArticulationAndBridges(dependencyState) {
    const nodes = [...dependencyState.files].sort();
    ...
    const dfs = (node) => {
        ...
        time += 1;
        let children = 0;
        for (const next of adjacency.get(node) || new Set()) {
            if (!visited.has(next)) {
                children += 1;
...
export function computeChokepoints(dependencyState, dependencySummary, fileCriticalityByPath, sccClusters) {
    ...
    for (const chain of dependencySummary.criticalPaths || []) {
        for (const file of chain.path)
            criticalPathFiles.add(file);
    }
    ...
    return [...dependencyState.files]
        ...
        const score = Math.round(fanIn * 3 +
            fanOut * 1.2 +
            criticality / 10 +
            (articulation ? 12 : 0) +
            bridgeCount * 4 +
        ...
        const reasons = [];
        if (fanIn >= 8)
            reasons.push(`high fan-in (${fanIn})`);
        if (fanOut >= 6)
            reasons.push(`high fan-out (${fanOut})`);
        if (articulation)
            reasons.push('articulation point');
        if (bridgeCount > 0)
            reasons.push(`${bridgeCount} bridge edge(s)`);
...
export function computeGraphAnalytics(dependencyState, dependencySummary, fileCriticalityByPath) {
...
export function buildAdvancedGraphFindings(graphAnalytics, dependencyState, fileSummaries) {
    const findings = [];
    ...
    for (const point of graphAnalytics.chokepoints.slice(0, 8)) {
        if (point.fanIn < 3 || point.fanOut < 3 || point.score < 18)
            continue;
    ...
    for (const entry of fileSummaries) {
        ...
        if (!point || point.fanIn < 8 || point.score < 18)
            continue;
