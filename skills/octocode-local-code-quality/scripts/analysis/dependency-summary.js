...
export function buildDependencySummary(dependencyState, fileCriticalityByPath, options) {
    ...
    const criticalNodes = allFiles
        .map(file => ({
        ...dependencyProfileToRecord(file, dependencyState),
        ...(fileCriticalityByPath.get(file) || {}),
    }))
        .filter(node => (node.score || 0) > 12 ||
    ...
    return {
        ...
        unresolvedSample: unresolvedEdgeCount > 0
...
export function computeDependencyCriticalPaths(dependencyState, fileCriticalityByPath, options) {
    ...
    const walk = (file) => {
        ...
        for (const dep of edges) {
            ...
            if (candidateScore > bestPath.score ||
