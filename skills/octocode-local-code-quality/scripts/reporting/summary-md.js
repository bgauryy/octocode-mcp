...
export function computeHealthScore(findings, totalFiles) {
    return computeHealthScoreFromSeverityBreakdown(severityBreakdown(findings), totalFiles);
}
function computeHealthScoreFromSeverityBreakdown(breakdown, totalFiles) {
    if (totalFiles === 0)
        return 100;
    ...
    return Math.max(0, Math.min(100, Math.round(100 / (1 + weightedFindingsPerFile / 10))));
}
...
function summarizeActiveFeatures(activeFeatures) {
...
function isPillarActive(pillarKey, activeFeatures) {
    if (!activeFeatures)
        return true;
...
function formatCliPath(filePath) {
...
export function generateSummaryMd(opts) {
    ...
    lines.push('');
    const totalBefore = overallFindingStats.totalFindings || agentOutput
        ?.totalBeforeTruncation;
    const dropped = agentOutput
        ?.droppedCategories;
    ...
    const overallHealth = computeHealthScoreFromSeverityBreakdown(overallFindingStats.severityBreakdown, totalFiles);
    const archHealth = computeHealthScoreFromSeverityBreakdown(archStats?.severityBreakdown ?? severityBreakdown(architectureFindings), totalFiles);
    const qualHealth = computeHealthScoreFromSeverityBreakdown(qualStats?.severityBreakdown ?? severityBreakdown(codeQualityFindings), totalFiles);
    const deadHealth = computeHealthScoreFromSeverityBreakdown(deadStats?.severityBreakdown ?? severityBreakdown(deadCodeFindings), totalFiles);
    const secHealth = computeHealthScoreFromSeverityBreakdown(secStats?.severityBreakdown ?? severityBreakdown(securityFindings), totalFiles);
    const testHealth = computeHealthScoreFromSeverityBreakdown(testStats?.severityBreakdown ?? severityBreakdown(testQualityFindings), totalFiles);
    lines.push('## Health Scores\n');
    lines.push('| Pillar | Score | Grade |');
    lines.push('|--------|-------|-------|');
    const grade = (s) => s >= 80 ? 'A' : s >= 60 ? 'B' : s >= 40 ? 'C' : s >= 20 ? 'D' : 'F';
    const pushHealthRow = (label, pillarKey, score) => {
    ...
    const pushPillarSummary = (pillarKey, findingsCount, score, artifactKey, artifactName) => {
        if (!isPillarActive(pillarKey, activeFeatures)) {
            lines.push('> skipped by feature filter\n');
            return;
        }
        if (artifactKey && outputFiles[artifactKey]) {
            lines.push(`> ${findingsCount} findings (score: ${score}/100) — see [\`${artifactName}\`](./${outputFiles[artifactKey]})\n`);
            return;
        }
        if (artifactName) {
            lines.push(`> ${findingsCount} findings (score: ${score}/100) — no \`${artifactName}\` written for this scan\n`);
            return;
        }
    ...
    pushPillarSummary('architecture', archStats?.totalFindings ?? architectureFindings.length, archHealth, 'architecture', 'architecture.json');
    if (depGraph) {
    ...
    pushPillarSummary('code-quality', qualStats?.totalFindings ?? codeQualityFindings.length, qualHealth, 'codeQuality', 'code-quality.json');
    renderPillarCategories('code-quality', codeQualityFindings);
    lines.push('## Dead Code & Hygiene\n');
    pushPillarSummary('dead-code', deadStats?.totalFindings ?? deadCodeFindings.length, deadHealth, 'deadCode', 'dead-code.json');
    renderPillarCategories('dead-code', deadCodeFindings);
    lines.push('## Security\n');
    pushPillarSummary('security', secStats?.totalFindings ?? securityFindings.length, secHealth, 'security', 'security.json');
    renderPillarCategories('security', securityFindings);
    lines.push('## Test Quality\n');
    pushPillarSummary('test-quality', testStats?.totalFindings ?? testQualityFindings.length, testHealth, 'testQuality', 'test-quality.json');
    ...
    if (outputFiles.astTrees) {
        ...
        lines.push(`Run these commands from the skill directory. Current scan: \`${relativeScanDir}\`.\n`);
        lines.push('```');
        ...
        lines.push(`- Find functions: \`node scripts/ast-tree-search.js -i ${astTreeArg} -k function_declaration --limit 25\``);
        lines.push(`- Find classes: \`node scripts/ast-tree-search.js -i ${astTreeArg} -k class_declaration --limit 25\``);
        lines.push(`- Find control flow: \`node scripts/ast-tree-search.js -i ${astTreeArg} -p 'IfStatement|SwitchStatement|ForStatement|WhileStatement' --limit 25\``);
        lines.push(`- Narrow to one file: \`node scripts/ast-tree-search.js -i ${astTreeArg} --file "${exampleFileFilter}" -k function_declaration --limit 10\``);
        lines.push(`- Raw text fallback: \`rg 'FunctionDeclaration|IfStatement' ${astTreeArg}\``);
        lines.push('');
    }
