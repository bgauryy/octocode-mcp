...
import { computeReportAnalysisSummary, enrichFileInventoryEntries, enrichFindings, } from './analysis.js';
...
export function writeMultiFileReport(dir, report, options, dependencyState, dependencySummary, fileCriticalityByPath) {
    ...
    const graphAnalytics = report.graphAnalytics ??
        computeGraphAnalytics(dependencyState, dependencySummary, fileCriticalityByPath);
    ...
    const reportAnalysis = report.reportAnalysis ??
        computeReportAnalysisSummary(allFindings, enrichedFileInventory, hotFiles, graphAnalytics);
...
export function generateMermaidGraph(dependencyState, dependencySummary, _fileCriticalityByPath) {
    const lines = [];
