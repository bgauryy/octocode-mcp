...
export function detectLowAssertionDensity(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (!isTestFile(entry.file) || !entry.testProfile)
            continue;
        const { testBlocks } = entry.testProfile;
        if (testBlocks.length === 0)
            continue;
        const totalAssertions = testBlocks.reduce((sum, t) => sum + t.assertionCount, 0);
        ...
        if (ratio < 1) {
            findings.push({
                ...
                impact: 'Low assertion density means tests pass without verifying behavior — bugs slip through with false confidence.',
...
export function detectTestNoAssertion(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (!isTestFile(entry.file) || !entry.testProfile)
            continue;
        for (const block of entry.testProfile.testBlocks) {
            if (block.assertionCount === 0) {
                findings.push({
                    ...
                    reason: 'A test without assertions always passes. It provides no verification of behavior.',
                    files: [entry.file],
                    suggestedFix: {
                    ...
                    impact: 'Zero-assertion tests always pass — they provide no safety net and create a false sense of coverage.',
...
export function detectExcessiveMocking(fileSummaries, mockThreshold = 10) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (!isTestFile(entry.file) || !entry.testProfile)
            continue;
        ...
        if (mockCalls.length > mockThreshold) {
            findings.push({
                ...
                reason: 'Excessive mocking couples tests to implementation details, making them brittle and hard to maintain.',
                files: [entry.file],
                suggestedFix: {
                ...
                impact: 'Heavy mocking couples tests to implementation details — any refactor breaks them even if behavior is unchanged.',
...
export function detectSharedMutableState(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (!isTestFile(entry.file) || !entry.testProfile)
            continue;
        for (const decl of entry.testProfile.mutableStateDecls) {
            findings.push({
                ...
                reason: 'let/var at describe scope creates shared mutable state between tests. Tests may pass or fail depending on execution order.',
                files: [entry.file],
                suggestedFix: {
                    strategy: 'Move variable declaration inside each test or use beforeEach.',
                    steps: [
                ...
                impact: 'Shared mutable state causes order-dependent test results — tests pass in isolation but fail or flake in suite runs.',
...
export function detectMissingTestCleanup(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (!isTestFile(entry.file) || !entry.testProfile)
            continue;
        ...
        if (hasBeforeAll && !hasAfterAll) {
            ...
            findings.push({
                ...
                reason: 'Setup in beforeAll without teardown in afterAll can leak state (open connections, modified globals, temp files) across test suites.',
                files: [entry.file],
                suggestedFix: {
                    strategy: 'Add afterAll() to clean up resources allocated in beforeAll().',
                    steps: [
                        'Identify what beforeAll() sets up (connections, mocks, temp state).',
                        'Add afterAll() to tear it down.',
                    ],
                },
                impact: 'Missing teardown leaks resources (connections, file handles, globals) that poison subsequent test suites.',
        ...
        if (hasBeforeEach && !hasAfterEach) {
            ...
            findings.push({
                ...
                reason: 'Setup in beforeEach without teardown in afterEach can accumulate side effects across tests.',
                files: [entry.file],
                suggestedFix: {
                    strategy: 'Add afterEach() to clean up resources allocated in beforeEach().',
                    steps: [
                        'Identify what beforeEach() sets up.',
                        'Add afterEach() to tear it down or restore state.',
                    ],
                },
                impact: 'Per-test setup without teardown accumulates side effects, causing cascading failures in later tests.',
...
export function detectFocusedTests(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (!isTestFile(entry.file) || !entry.testProfile)
            continue;
        for (const focused of entry.testProfile.focusedCalls) {
            findings.push({
                ...
                impact: 'Focused tests can create a false green signal by skipping broader test coverage.',
...
export function detectFakeTimersWithoutRestore(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (!isTestFile(entry.file) || !entry.testProfile)
            continue;
        const fakeTimerCalls = entry.testProfile.timerControls.filter((call) => call.kind === 'jest.useFakeTimers' || call.kind === 'vi.useFakeTimers');
        if (fakeTimerCalls.length === 0)
            continue;
        const hasRestore = entry.testProfile.timerControls.some((call) => call.kind === 'jest.useRealTimers' || call.kind === 'vi.useRealTimers');
        if (hasRestore)
            continue;
        ...
        findings.push({
            ...
            reason: 'Tests that switch to fake timers without restoring real timers can leak timing behavior into subsequent tests.',
            files: [entry.file],
            suggestedFix: {
                strategy: 'Pair fake timer activation with a restore in the same test scope.',
                steps: [
            ...
            impact: 'Leaked fake-timer configuration can cause subtle, order-dependent failures across unrelated suites.',
            tags: ['test-quality', 'timers', 'isolation'],
            ruleId: 'test-quality.fake-timer-no-restore',
            confidence: 'medium',
            evidence: {
                fakeTimerActivationLines: fakeTimerCalls.map((call) => `${call.kind}:${call.lineStart}`),
            },
        });
    }
    return findings;
}
export function detectMissingMockRestoration(fileSummaries) {
    const findings = [];
    for (const entry of fileSummaries) {
        if (!isTestFile(entry.file) || !entry.testProfile)
            continue;
        if (entry.testProfile.spyOrStubCalls.length === 0)
            continue;
        const hasRestoreAll = entry.testProfile.mockRestores.some((call) => call.kind === 'restoreAll');
        if (hasRestoreAll)
            continue;
        const explicitRestores = new Set(entry.testProfile.mockRestores
            .filter((call) => call.kind === 'restore' && !!call.target)
            .map((call) => call.target));
        const firstUnrestored = entry.testProfile.spyOrStubCalls.find((call) => !call.target || !explicitRestores.has(call.target));
        if (!firstUnrestored)
            continue;
        ...
        findings.push({
            ...
            reason: 'Spies/stubs modify implementation behavior and must be restored to avoid cross-test leakage.',
            files: [entry.file],
            suggestedFix: {
                strategy: 'Restore every spy/stub after each test or in a file-level teardown.',
                steps: [
                    'Call `mockRestore()` on each spy/stub returned handle.',
                    'Or use `jest.restoreAllMocks()`/`vi.restoreAllMocks()` in afterEach/afterAll.',
                ],
            },
            impact: 'Unrestored spies/stubs make tests order-dependent and can mask regressions.',
            tags: ['test-quality', 'cleanup', 'mocks', 'isolation'],
            ruleId: 'test-quality.missing-mock-restoration',
            confidence: 'high',
            evidence: {
                spyOrStubCalls: entry.testProfile.spyOrStubCalls.map((call) => `${call.lineStart}`),
            },
        });
    }
    return findings;
