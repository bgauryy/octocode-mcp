...
export interface SanitizationOptions {
  ...
  enableMaliciousPatterns?: boolean;
  enableSuspiciousRepetitionCheck?: boolean;
}
...
export class ContentSanitizer {
  ...
  public static sanitizeContent(
    ...
    const {
      ...
      enableMaliciousPatterns = false,
      enableSuspiciousRepetitionCheck = false,
    } = opts;
    ...
    if (enableSuspiciousRepetitionCheck) {
  ...
  private static sanitizeParameter(value: string): string {
    // Remove characters that are dangerous for command injection
    // Including: semicolon, pipe, backtick, dollar, parentheses, braces, brackets, backslash
    // AND: ampersand, newlines, carriage returns, null bytes
    // BUT: keep < and > as they're safe for CLI date ranges and comparisons
    let sanitized = value.replace(/[;|`$(){}[\]\\&\n\r\0]/g, '');

    // Additional security: remove control characters and other dangerous sequences
    // eslint-disable-next-line no-control-regex
    sanitized = sanitized.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, '');

    // Remove dangerous escape sequences
    sanitized = sanitized.replace(/\\[nrtbfav0]/g, '');

    // Remove multiple consecutive spaces/tabs (potential obfuscation)
