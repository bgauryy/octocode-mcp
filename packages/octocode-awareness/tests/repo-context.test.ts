...
it('rejects removed query aliases before they can change the workspace scope', async () => {
    ...
    try {
      ...
      expect(queryAwareness(db, { view: 'memories', workspacePath: '/workspace/alpha' }).rows)
        .toHaveLength(1);
      expect(() => queryAwareness(db, {
        view: 'memories', workspace_path: '/workspace/beta',
      } as unknown as AwarenessQueryParams)).toThrow(/queryAwareness: unknown options: workspace_path/);
    } finally {
      db.close();
    }
  });
...
it('queries every view and renders all supported formats', async () => {
    ...
    try {
      ...
      const signals = queryAwareness(db, {
        ...base,
        view: 'signals',
        agentId: 'agent-a',
        includeBodies: true,
      });
      expect(signals.rows).toContainEqual(expect.objectContaining({ from_agent: 'agent-a', body: 'schema first' }));
...
it('builds a delta-sized compact attend packet with only actionable work', async () => {
    ...
    try {
      ...
      expect(result.next.target?.run_id).toBe('run_pending_a');
...
it('clusters repeated handoff signals in compact attend packets', () => {
    ...
    try {
      ...
      expect(result.next.action).toBe('inspect_inbox');
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
it('keeps global Verify totals exact while routing only the current agent', async () => {
    ...
    try {
      ...
      expect(peer.next.action).not.toBe('verify_owned_work');
      ...
      expect(owner.next.action).toBe('verify_owned_work');
      expect(owner.next).not.toHaveProperty('command');
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
it('routes a submitted task to its exact pending run owner', () => {
    ...
    try {
      ...
      expect(worker.next).toMatchObject({ action: 'verify_owned_work', target: { run_id: 'run_verify_exact' } });
      expect(worker.next).not.toHaveProperty('command');
      const lead = attendAwareness(db, { agentId: 'lead', workspacePath: dir, compact: true });
      expect(lead.next.action).not.toBe('verify_owned_work');
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
