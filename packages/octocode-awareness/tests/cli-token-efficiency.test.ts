...
describe('CLI token efficiency', () => {
  ...
  it('routes an inbox to a bounded signal read', () => {
    ...
    try {
      ...
      expect(attend['next']).toMatchObject({ action: 'inspect_inbox', command: {
        name: 'signal list', args: expect.arrayContaining(['--limit', '3', '--db', realpathSync(db)]),
      } });
  ...
  it('keeps discovery, exact contracts, and hooks receipts byte-lean', () => {
    ...
    try {
      ...
      expect(hooks.parsed).toMatchObject({ ok: true, action: 'dry-run', host: 'claude', profile: 'coordination', hook_count: 5 });
