...
describe('bounded ranked memory recall', () => {
  it('exposes omitted ranked results instead of claiming a complete result', async () => {
    const db = connectDb(':memory:');
    try {
      for (let i = 0; i < 8; i++) (await insertMemory(db, { taskContext: 'limits', observation: `shared constraint ${i}`, importance: 5 }));
      const bounded = (await getMemory(db, { query: 'constraint', limit: 2, recordAccess: false }));
      ...
      const complete = (await getMemory(db, { query: 'constraint', limit: 20, recordAccess: false }));
