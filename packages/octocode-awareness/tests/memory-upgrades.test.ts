...
async function seedCorpus(db: DatabaseSync): Promise<void> {
  (await insertMemory(db, { taskContext: 'auth router refactor', observation: 'tenant id normalization order matters for policy lookup', importance: 7, label: 'GOTCHA', tags: ['auth'] }));
  (await insertMemory(db, { taskContext: 'build pipeline', observation: 'esbuild banner suppresses the sqlite warning noise', importance: 5, label: 'BUILD' }));
  (await insertMemory(db, { taskContext: 'release process', observation: 'prepack guard rejects workspace protocol leaks', importance: 6, label: 'RELEASE' }));
  (await insertMemory(db, { taskContext: 'unrelated note', observation: 'completely different topic entirely', importance: 3, label: 'OTHER' }));
}
...
describe('weak-pool relevance squash', () => {
  ...
  it('a strong match scores high but never a perfect 1.0', { timeout: 20_000 }, async () => {
    const db = freshDb();
    (await seedCorpus(db));
    const { memories } = (await getMemory(db, { query: 'tenant normalization order', limit: 3, explain: true }));
  ...
  it('degenerate bm25 pools (near-empty store) get neutral 0.5, not inflated 1.0', async () => {
    const db = freshDb();
    (await insertMemory(db, { taskContext: 'auth router', observation: 'tenant order matters', importance: 5 }));
    const { memories } = (await getMemory(db, { query: 'auth router', limit: 1 }));
...
describe('judgment_required — low-confidence recall flag', () => {
  it('is absent on a confident match', async () => {
    const db = freshDb();
    (await seedCorpus(db));
    const res = (await getMemory(db, { query: 'tenant normalization order', limit: 3 }));
    expect(res.judgment_required).toBeUndefined();
  });

  it('fires with a reason when the query matches nothing', async () => {
    const db = freshDb();
    (await seedCorpus(db));
    const res = (await getMemory(db, { query: 'kubernetes ingress annotation', limit: 3 }));
    expect(res.count).toBe(0);
    expect(res.judgment_required).toBe(true);
    expect(res.judgment_reason).toContain('absence');
  });

  it('does not fire on empty-query browsing', async () => {
    const db = freshDb();
    (await seedCorpus(db));
    const res = (await getMemory(db, { query: '', limit: 3 }));
    expect(res.judgment_required).toBeUndefined();
  });
});
...
describe('per-label decay half-life defaults', () => {
  ...
  it('durable labels get 90d, EXPERIENCE 14d, others store default (NULL → 30d at read)', async () => {
    const db = freshDb();
    const decision = (await insertMemory(db, { taskContext: 't', observation: 'a', importance: 8, label: 'DECISION' })).memoryId;
    const gotcha = (await insertMemory(db, { taskContext: 't', observation: 'b', importance: 8, label: 'GOTCHA' })).memoryId;
    const exp = (await insertMemory(db, { taskContext: 't', observation: 'c', importance: 5, label: 'EXPERIENCE' })).memoryId;
    const bug = (await insertMemory(db, { taskContext: 't', observation: 'd', importance: 5, label: 'BUG' })).memoryId;
...
describe('salience floor on broad forget selectors', () => {
  it('a broad --before sweep skips high-importance memories and reports the floor', async () => {
    const db = freshDb();
    const critical = (await insertMemory(db, { taskContext: 'critical', observation: 'data-loss rule', importance: 9 })).memoryId;
    const minor = (await insertMemory(db, { taskContext: 'minor', observation: 'small note', importance: 3 })).memoryId;
  ...
  it('explicit --max-importance overrides the floor; explicit ids bypass it', async () => {
    const db = freshDb();
    const critical = (await insertMemory(db, { taskContext: 'critical', observation: 'rule', importance: 9 })).memoryId;
  ...
  it('scopes broad forget selectors to the requested workspace', async () => {
    const db = freshDb();
    ...
    const global = (await insertMemory(db, {
      taskContext: 'global',
