...
| Signals and continuation | Publish, list, reply and dry-run prune with `--workspace "$SWEEP_WS"`. Acknowledge by signal ID and resolve by thread ID in the same database; these two CLI routes do not accept `--workspace`. Create a session capture or refinement only when continuation is real. | The thread keeps its identity and participant history. Linked worktrees share signals; unrelated clones remain isolated. Acknowledgement means handled; resolution closes completed threads. |
