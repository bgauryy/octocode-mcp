...
export function runDatabaseCommand(argv: string[]): number {
  ...
  if (action === 'consolidate' && rest.includes('--help')) {
    process.stdout.write('Usage: octocode-awareness database consolidate --source <existing-file> --destination <new-file> [--unattributed-agent-id <actor>] [--dry-run] [--compact]\nschema: database_consolidate\nexample: octocode-awareness database consolidate --source old.sqlite3 --destination new.sqlite3 --dry-run\nConverts into a new file. The source is read-only; incomplete contracts and collisions are errors. --dry-run validates a private copy and discards it without publishing the destination.\n');
    return 0;
  }
  ...
  try {
    ...
    const allowed = new Set(['source', 'destination', 'unattributed_agent_id', 'dry_run', 'compact', '_']);
    ...
    print({ ok: true, ...consolidateDatabase(source, destination, { unattributedAgentId, dryRun: args['dry_run'] === true }) });
    return 0;
  } catch (error) {
