...
import { agentId, db, sessionId, shellHookHost, workspace } from './hook-payload.js';
import type { normalizeToolHookPayload } from './hook-tool-protocol.js';
...
export async function captureHookHistory(
  ...
  try {
    ...
    console.error(`octocode-awareness history capture warning (continuing): ${(error as Error).message}`);
  } finally {
    historyDb?.close();
  }
}
