...
import type { WorkPeer } from '../types/work-maintenance.js';
import { INTERNAL_HOOK_HOST, agentHost, agentId, agentName, agentVendor, artifact, resolveHookPath, workspace } from './payload.js';
import { overlapChangeSignal } from './signals.js';
...
export function leaveHookAgent(database: DatabaseSync, payload: Record<string, unknown>): void {
  const cwd = workspace(payload) ?? process.cwd();
  database.prepare("UPDATE awareness_agents SET status = 'LEFT' WHERE workspace_path = ? AND agent_id = ?")
    .run(normalizeWorkspacePath(cwd, cwd) ?? resolve(cwd), agentId(payload));
}
