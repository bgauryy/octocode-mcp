...
import { activeTaskClaimForAgent } from '../tasks-catalog.js';
import { endWork, startWork, touchWork } from '../work.js';
import { discardUncommittedHookFiles } from '../work-hook.js';
import { evaluateHarnessGuard } from '../harness-guard.js';
import { agentId, artifact, completeHookControl, db, emitHookContext, extractFiles, hookBlockOutcome, hookSkillRoot, hookToolFailed, resolveHookPath, shellHookHost, workspace } from './payload.js';
import { emitPeerDelta, registerHookAgent } from './peers.js';
import { activeRunForFiles, consumeHookRun, isAggregatedFallbackHookRun, recordHookRun, refreshFallbackVerificationPlan, runOrigin, startOrAttachFallbackHookRun, withHookDbRetry } from './run-state.js';
...
export async function runPreEdit(
  ...
  try {
    ...
    writeCommandDiagnostic(`octocode-awareness pre-flight warning (continuing): ${error instanceof Error ? error.message : String(error)}`);
    return 0;
  }
}
...
export async function runPostEdit(payload: Record<string, unknown>): Promise<number> {
  ...
  try {
    ...
    if (!correlatedRunId) {
      writeCommandDiagnostic('octocode-awareness post-edit warning (continuing): could not identify a unique work run; leaving presence for expiry.');
      return 0;
    }
    ...
    writeCommandDiagnostic(`octocode-awareness post-edit warning during ${stage} (continuing): ${error instanceof Error ? error.message : String(error)}`);
  }
  return 0;
}
