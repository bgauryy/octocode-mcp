...
export function workboardRows(db: DatabaseSync, params: AwarenessQueryParams): AwarenessQueryRow[] {
  ...
  for (const row of runRows(db, withScope(params, { state: ['PENDING'], limit: 500 }), { standaloneOnly: true })) {
    verificationRows.push({
      item_type: 'run',
  ...
  if (params.preferAgentId) verificationRows.sort((left, right) =>
    Number(right['agent_id'] === params.preferAgentId) - Number(left['agent_id'] === params.preferAgentId));
  for (const row of verificationRows) pushLimited(columns, counts, 'Verify', row, limit);
  ...
  const pressure = inspectMaintenancePressure(db, {
    workspace_path: pressureScope.workspacePath,
