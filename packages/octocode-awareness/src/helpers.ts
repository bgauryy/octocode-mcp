...
/** Reject misspelled options before a dropped scope can broaden an operation. */
export function assertKnownOptions(value: object, allowed: readonly string[], label: string): void {
  const unknown = Object.keys(value).filter((key) => !allowed.includes(key));
  if (unknown.length > 0) throw new Error(`${label}: unknown options: ${unknown.join(', ')}`);
}
