...
export function peerBriefing(db: DatabaseSync, params: {
  ...
  const signals = inbox.signals.map(signal => {
    const clipped = signal.subject.length > 200 || (signal.body?.length ?? 0) > 800;
    ...
    return {
      ...
      subject: signal.subject.slice(0, 200), body: signal.body?.slice(0, 800) ?? null,
  ...
  const packet = {
    signals,
    partial: Boolean(inbox.partial || clippedIds.length),
    partialReasons: [...(inbox.partial ? ['limit'] : []), ...(clippedIds.length ? ['message_length'] : [])],
    next: {
      ...(cursor ? { list: { command: { name: 'signal list', args: [...scopeArgs, '--cursor', String(cursor), '--limit', '5', '--include-bodies'] } } } : {}),
      ...(clippedIds.length ? { read: { command: { name: 'signal list', args: [...scopeArgs, '--all', '--include-bodies',
        ...clippedIds.flatMap(id => ['--signal-id', id])] } } } : {}),
    },
  };
