...
export function scopeFromParams(params: AwarenessQueryParams): Scope {
  ...
  const rawWorkspace = params.workspacePath ?? cwd;
  const workspacePath = rawWorkspace ? resolve(String(rawWorkspace)) : null;
  const scope = {
    // Keep the raw resolved path for projection output / echo; the normalized
    // path set below carries the keys used for DB row matching.
    workspacePath,
