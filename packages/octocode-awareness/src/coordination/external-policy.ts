...
- Discover only when needed: use compact tiers for routine routes, \`schema commands --all --compact\` for the full catalog, and \`schema command <noun> [action] --compact\` for one exact route. Memory/history maintenance and recovery stay advanced but remain callable by exact name. Use the native facade or bound CLI when the facade is unavailable. Keep host bindings separate; reuse context for returned calls. Copy exact IDs, revisions, cursors, preview IDs, and continuations; do not reconstruct them. Load the relevant recipe. Features are optional; coordinate overlap and never bypass a peer lock.
