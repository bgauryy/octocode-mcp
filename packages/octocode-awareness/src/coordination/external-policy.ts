...
- Recall memory only when prior learning could change the approach. Prefer a supplied ID/digest to search. Reuse retrieved evidence in the same scope while valid; fetch again only after change, expiry or a new unresolved question. Save one reusable reason/constraint with scope and evidence, not status or repeated lessons. Describe the chosen memory write once for provenance and ownership rules.
