...
- Run the declared check, then use \`work end\` or task submit to move the run to PENDING before \`verify mark\`; pass explicit run IDs and observed --status SUCCESS or --status FAILED. Unrun checks stay PENDING; closure, expiry, exit, capture and acknowledgement are not success. Before the final response, \`verify audit\` your identity/workspace and owned workers after their last artifact/terminal write. Settle only observed checks or disclose debt; preserve unrelated peer debt. Record reusable learning with memory or \`reflect record\` only after verification; recheck stale/unknown evidence before reuse.
...
5. Do the work.
6. After the declared check has an observed result, awareness(call "work end") or awareness(call "task submit") — close the run and move it to PENDING; closure alone does not prove success.
7. awareness(call "verify mark") — mark the PENDING run with observed status SUCCESS or FAILED. Expiry, exit, closure, and ack never prove verification.
