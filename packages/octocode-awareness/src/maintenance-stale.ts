...
export function pruneStale(db: DatabaseSync, params: Record<string, unknown> = {}): PruneStaleResult {
  assertKnownOptions(params, [
    'dry_run', 'expired_only', 'older_than_minutes', 'agent_id', 'workspace_path', 'artifact', 'target_file',
  ], 'prune stale locks');
  const dryRun = Boolean(params.dry_run);
  const expiredOnly = Boolean(params.expired_only);
  const olderThanMinutes = params.older_than_minutes != null ? Number(params.older_than_minutes) : null;
  ...
  const rawTarget = params.target_file;
