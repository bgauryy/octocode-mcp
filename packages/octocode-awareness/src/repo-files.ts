...
export function filesUnderWorkRows(db: DatabaseSync, params: AwarenessQueryParams): AwarenessQueryRow[] {
  ...
  const preferredPaths = [...new Set((params.preferFiles ?? []).map(file => resolve(scope.workspacePath ?? process.cwd(), file)))];
  const preferredOrder = preferredPaths.length > 0
    ? `CASE WHEN rf.file_path IN (${preferredPaths.map(() => '?').join(',')}) THEN 0 ELSE 1 END, locked DESC, `
    : '';
  const rows = db.prepare(
    `SELECT rf.file_path, rf.run_id, tr.agent_id, tr.task_id,
      ...
      ORDER BY ${preferredOrder}rf.file_path, datetime(rf.heartbeat_at) DESC, tr.agent_id, rf.run_id
      LIMIT ?`
  ).all(utcNow(), ...binds, ...preferredPaths, rowLimit) as unknown as Array<Record<string, string | number | null>>;
