 ...
 *
 * Schema: canonical `awareness_agents` table.
 */
...
export function registerAgent(
  ...
  const workspacePath = params.workspacePath ? normalizeWorkspacePath(params.workspacePath, params.workspacePath) ?? params.workspacePath : '';
...
export function touchAgent(db: DatabaseSync, agentId: string, workspacePath: string | null = null, artifact: string | null = null): void {
  try {
    ...
    db.prepare(AGENTS_UPSERT).run(
      ...
      prior.agent_name,
      normalized,
      normalizeArtifact(artifact) ?? prior.artifact,
      prior.context,
      stamp,
      stamp,
      prior.metadata_json,
...
export function resolveAgentNames(
  ...
  try {
    ...
    for (const row of rows) if (!result.has(row.agent_id)) result.set(row.agent_id, row.agent_name);
  } catch { /* ignore */ }
  return result;
}
 ...
 * Optionally filter to a workspace; unscoped registrations remain visible.
 *
...
export function listAgents(
  ...
  try {
    ...
    const agents = rows.map((row) => ({ ...row, workspace_path: row.workspace_path || null }));
