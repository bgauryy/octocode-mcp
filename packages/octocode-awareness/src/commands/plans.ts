...
import { firstValue, listLimit, valuesFor } from './args.js';
...
export function cmdPlan(db: DatabaseSync, args: ParsedArgs, dbPath: string, opts: EmitOptions): number {
  ...
  if (action === 'create') {
    ...
    const result = createPlan(db, {
      ...
      leadAgentId: String(args['lead_agent_id'] ?? args['agent_id'] ?? '').trim(),
  ...
  if (action === 'join') {
    const member = joinPlan(db, { planId, agentId: String(args['agent_id'] ?? '').trim() });
...
export function cmdTask(db: DatabaseSync, args: ParsedArgs, dbPath: string, opts: EmitOptions): number {
  ...
  if (action === 'list' || action === 'ready') {
    ...
    const filters = {
        ...
        status: args['status'] ? String(args['status']).toUpperCase() as TaskStatus : null,
