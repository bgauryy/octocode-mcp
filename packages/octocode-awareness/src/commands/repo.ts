...
export function cmdDocsCatalog(args: ParsedArgs, opts: EmitOptions): number {
  ...
  if (action === 'show') {
    ...
    writeCommandText(`${result.content}${result.content.endsWith('\n') ? '' : '\n'}`);
    return 0;
  }
