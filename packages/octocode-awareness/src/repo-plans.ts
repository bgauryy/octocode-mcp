...
export function taskRows(db: DatabaseSync, params: AwarenessQueryParams): AwarenessQueryRow[] {
  ...
  const sqlWhere = where.length > 0 ? `WHERE ${where.join(' AND ')}` : '';
  const preferredOrder = params.preferAgentId ? 'CASE WHEN COALESCE(claimed_by, t.created_by) = ? THEN 0 ELSE 1 END, ' : '';
  const preferredBinds = params.preferAgentId ? [params.preferAgentId] : [];
  const rows = db.prepare(
    `SELECT t.*, p.name AS plan_name, p.status AS plan_status, p.workspace_path, p.artifact,
       ...
       ${sqlWhere}
      ORDER BY ${preferredOrder}t.priority DESC, datetime(t.created_at), t.task_id
      LIMIT ?`
  ).all(...binds, ...preferredBinds, limitOf(params.limit)) as unknown as Array<Record<string, string | number | null>>;
...
export function runRows(db: DatabaseSync, params: AwarenessQueryParams, options: { standaloneOnly?: boolean } = {}): AwarenessQueryRow[] {
  const scope = scopeFromParams(params);
  const where: string[] = [];
  const binds: BindValue[] = [];
  ...
  if (options.standaloneOnly) where.push('tr.task_id IS NULL');
  const agentId = params.agentId;
  if (agentId) { where.push('tr.agent_id = ?'); binds.push(agentId); }
  const since = params.since?.trim();
  if (since) { where.push('tr.created_at >= ?'); binds.push(since); }
  const sqlWhere = where.length > 0 ? `WHERE ${where.join(' AND ')}` : '';
  const preferredOrder = params.preferAgentId ? 'CASE WHEN tr.agent_id = ? THEN 0 ELSE 1 END, ' : '';
  const preferredBinds = params.preferAgentId ? [params.preferAgentId] : [];
  const rows = db.prepare(
    `SELECT tr.run_id, tr.task_id, tr.agent_id, tr.session_id, tr.rationale, tr.test_plan,
      ...
      ORDER BY ${preferredOrder}datetime(tr.created_at) DESC LIMIT ?`
  ).all(...binds, ...preferredBinds, limitOf(params.limit)) as unknown as Array<Record<string, string | null>>;
  return rows.map(row => ({
