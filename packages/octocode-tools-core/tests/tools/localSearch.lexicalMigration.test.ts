...
describe('localSearch lexical contract', () => {
  ...
  beforeAll(async () => {
    ...
    root = await mkdtemp(join(process.cwd(), '.octocode', 'tmp', 'local-lexical-'));
  ...
  it.each([
    ...
    ['rust', 'needle'],
    ['pcre2', 'needle(?:ish)?'],
  ] as const)('maps regex:%s to the native lexical matcher', async (regex, searchText) => {
    const result = await executeDirectTool('localSearch', {
      queries: [{ path: root, searchText, regex, resultView: 'countMatches' }],
    });
    expect(result.isError).not.toBe(true);
    expect((result.structuredContent as { results: Array<{ data: { stats?: { totalOccurrences?: number } } }> }).results[0]?.data.stats?.totalOccurrences).toBeGreaterThan(0);
  });

  it('normalizes lexical continuations to schema-valid operation-free queries', async () => {
    const result = await executeDirectTool('localSearch', {
      queries: [{ path: root, searchText: 'needle', resultView: 'paginated', pageSize: 1 }],
    });
    const row = (result.structuredContent as { results: Array<{ data: Record<string, unknown> }> }).results[0];
    const next = (row?.data.next as Record<string, { query: Record<string, unknown> }> | undefined)?.nextPage?.query;
    if (next) {
      expect(next.operation).toBeUndefined();
      const parsed = LocalSearchQuerySchema.safeParse(next);
      expect(parsed?.success, JSON.stringify({ next, issues: parsed && !parsed.success ? parsed.error.issues : [] })).toBe(true);
    }
  });
});
