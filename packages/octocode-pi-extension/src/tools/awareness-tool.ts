...
export function registerAwarenessTool(
  ...
  registerFn(pi, registeredToolNames, {
    ...
    promptSnippet: 'Start with attend (lobby), recall before any edit, work start before writing, verify mark after check, work end to close, verify audit before the final response. Use list → describe → call for unfamiliar commands.',
    promptGuidelines: [
      ...
      'After work: call awareness("verify mark") only after the declared check ran with an observed status SUCCESS or FAILED — expiry, exit, and ack never prove verification.',
      'Close the run: call awareness("work end") or awareness("task submit") when done.',
