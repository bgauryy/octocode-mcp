...
test('every foreground activity has an explicit motion and status contract', () => {
  const cases = [
    ...
    [{ kind: 'planning', since: 1, planScope: 'w' } as const, true, 'Planning…'],
      [{ kind: 'reviewing', since: 1, planScope: 'w' } as const, false, '⏳ Review RFC — approve Start or Request changes'],
      [{ kind: 'awaiting_start', since: 1, planScope: 'w', revision: 'r' } as const, false, '⏳ Plan approved — run /octocode-plan start <revision>'],
