...
import { activeExecutionTools } from '../src/tools/execution-events.js';
...
describe('lifecycle UI', () => {
  it('tracks overlapping calls independently and restores plan activity without exposing arguments', async () => {
    ...
    expect(runtimeActivityPresentation(h.store.getState()).status).toBe('Bash · 2 tools');
    await h.fire('tool_execution_end', { toolCallId: 'a', toolName: 'MCPTool' });
    expect(runtimeActivityPresentation(h.store.getState()).status).toBe('Bash');
  ...
  it('gives input and compaction clear priority, then clears aborted work', async () => {
    ...
    emitExecution(h.ctx, 'question.requested', { id: 'ask:question:0', title: 'Answer the pending question' });
    expect(runtimeActivityPresentation(h.store.getState())).toMatchObject({ visible: false, status: 'Input needed · Answer the pending question', token: 'warning' });
    await h.fire('session_before_compact');
    expect(runtimeActivityPresentation(h.store.getState()).status).toBe('Input needed · Answer the pending question');
    ...
    expect(activeExecutionTools(h.store.getState().execution)).toEqual([]);
    h.dispose();
  });
