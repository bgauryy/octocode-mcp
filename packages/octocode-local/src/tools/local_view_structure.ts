...
import { validateToolPath } from '../utils/toolHelpers.js';
import type { ViewStructureQuery, ViewStructureResult } from '../types.js';
import fs from 'fs';
...
export async function viewStructure(
  ...
  try {
    // Use standardized validateToolPath helper for consistent error handling
    const pathValidation = validateToolPath(query, 'LOCAL_VIEW_STRUCTURE');
