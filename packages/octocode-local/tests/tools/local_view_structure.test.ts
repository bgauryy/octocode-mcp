...
describe('local_view_structure', () => {
  ...
  describe('Path validation', () => {
    ...
    it('should use validateToolPath helper for consistent error handling', async () => {
      // This test verifies that local_view_structure uses the same
      // validateToolPath helper as other tools for consistent error codes
      mockValidate.mockReturnValue({
        isValid: false,
      ...
      const result = await viewStructure({
        path: '../../../etc/passwd',
        researchGoal: 'Test error handling',
        reasoning: 'Verify consistent validation',
      });

      expect(result.status).toBe('error');
      expect(result.errorCode).toBe(ERROR_CODES.PATH_VALIDATION_FAILED);
      // Research context should be preserved in error results
      expect(result.researchGoal).toBe('Test error handling');
      expect(result.reasoning).toBe('Verify consistent validation');
      // Should have hints from getToolHints
