...
describe('local_fetch_content', () => {
  ...
  describe('Security: Use sanitizedPath from validation', () => {
    it('should use sanitizedPath from validateToolPath instead of re-resolving', async () => {
      // Mock pathValidator to return a specific sanitizedPath
      const sanitizedPath = '/workspace/safe/resolved/path.txt';
      mockValidate.mockReturnValue({
        isValid: true,
        sanitizedPath,
      });
      ...
      const result = await fetchContent({
        path: '../unsafe/../path.txt', // User provides potentially unsafe path
        fullContent: true,
      });

      expect(result.status).toBe('hasResults');
      // The file should be read using the sanitizedPath, not the original path
      expect(mockReadFile).toHaveBeenCalledWith(sanitizedPath, 'utf-8');
    });

    it('should use sanitizedPath for stat check as well', async () => {
      const sanitizedPath = '/workspace/safe/resolved/path.txt';
      mockValidate.mockReturnValue({
        isValid: true,
        sanitizedPath,
      ...
      // stat() should also use the sanitizedPath
      expect(mockStat).toHaveBeenCalledWith(sanitizedPath);
    });
  });
