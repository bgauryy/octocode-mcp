...
} from '../scheme/lsp_find_references.js';
import {
  SymbolResolver,
...
export function registerLSPFindReferencesTool(server: McpServer) {
  return server.registerTool(
    'lspFindReferences',
    {
      ...
      inputSchema: BulkLSPFindReferencesSchema,
      annotations: {
    ...
    async (args: {
      ...
      >(args.queries || [], async query => await findReferences(query), {
        toolName: TOOL_NAME,
      });
    }
  );
 ...
 * Fallback: Find references using pattern matching (ripgrep/grep)
 */
async function findReferencesWithPatternMatching(
  absolutePath: string,
  workspaceRoot: string,
  query: LSPFindReferencesQuery
): Promise<FindReferencesResult> {
