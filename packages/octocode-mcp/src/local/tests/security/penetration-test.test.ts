...
describe('🔴 SECURITY PENETRATION TEST - PathValidator', () => {
  const mockWorkspace = '/Users/guybary';
  ...
  describe('ATTACK 1: Path Traversal Variations', () => {
    it('should BLOCK classic ../ traversal', () => {
      const result = validator.validate('/Users/guybary/../../../etc/passwd');
      expect(result.isValid).toBe(false);
      expect(result.error).toContain('outside allowed directories');
    });

    it('should BLOCK deep traversal (10 levels)', () => {
      const path = '/Users/guybary' + '/..'.repeat(10) + '/etc';
    ...
    it('should BLOCK mixed ./ and ../', () => {
      const result = validator.validate(
        '/Users/guybary/./Documents/.././.././.././etc'
      );
      expect(result.isValid).toBe(false);
    });

    it('should BLOCK trailing slash traversal', () => {
      const result = validator.validate('/Users/guybary/../../../etc/');
      expect(result.isValid).toBe(false);
    });
  });
  ...
  describe('ATTACK 2: Encoding Attacks', () => {
    it('URL encoded dots (%2e%2e) - SAFE (Node treats as literal)', () => {
      ...
      const result = validator.validate('/Users/guybary/%2e%2e/%2e%2e/etc');
      ...
      expect(result.sanitizedPath?.startsWith('/Users/guybary')).toBe(true);
    });
    ...
    it('Double URL encoding (%252e) - SAFE (Node treats as literal)', () => {
      ...
      const result = validator.validate('/Users/guybary/%252e%252e/etc');
      expect(result.isValid).toBe(true);
      expect(result.sanitizedPath?.startsWith('/Users/guybary')).toBe(true);
    });
  });
  ...
  describe('ATTACK 3: Special Characters', () => {
    it('should BLOCK null byte injection', () => {
      const result = validator.validate('/Users/guybary\x00/../../../etc');
      expect(result.isValid).toBe(false);
    });
    ...
    it('should BLOCK carriage return', () => {
      const result = validator.validate('/Users/guybary\r/../../../etc');
      expect(result.isValid).toBe(false);
    });
  });
  ...
  describe('ATTACK 6: Sibling Directory Bypass', () => {
    it('should BLOCK sibling with similar name', () => {
      const result = validator.validate('/Users/guybary2');
      expect(result.isValid).toBe(false);
      expect(result.error).toContain('outside allowed directories');
    });

    it('should BLOCK sibling with extra characters', () => {
      const result = validator.validate('/Users/guybary_other');
      expect(result.isValid).toBe(false);
    });

    it('should BLOCK sibling with prefix match', () => {
      const result = validator.validate('/Users/guybaryextra/Documents');
      expect(result.isValid).toBe(false);
    });
  });
  ...
  describe('VALID PATHS - Should ALLOW', () => {
    it('should ALLOW valid workspace subdirectory', () => {
      const result = validator.validate('/Users/guybary/Documents');
      expect(result.isValid).toBe(true);
    ...
    it('should ALLOW deeply nested valid paths', () => {
      const result = validator.validate(
        '/Users/guybary/Documents/projects/app/src/components'
      );
      expect(result.isValid).toBe(true);
    });
  });
