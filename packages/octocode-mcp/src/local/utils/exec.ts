...
export async function safeExec(
  ...
  return new Promise((resolve, reject) => {
    ...
    // Wrap spawn in try-catch to handle spawn failures
    let child;
    try {
      ...
      reject(
        new Error(
          `Failed to spawn command '${command}': ${spawnError instanceof Error ? spawnError.message : String(spawnError)}`
        )
      );
      return;
    }
    ...
    timeoutHandle = setTimeout(() => {
      if (child.exitCode === null) {
        killed = true;
        child.kill('SIGKILL'); // Use SIGKILL for forceful termination
        cleanup();
