 ...
 * 2. GH_TOKEN env
 * 3. GITHUB_TOKEN env
 * 4. Keychain (stored credentials)
 * 5. File (encrypted fallback)
...
describe('Token Resolution Priority (TOKEN_RESOLUTION.md)', () => {
  ...
  describe('Priority Order Verification', () => {
    ...
    it('should use stored credentials when env is empty (Priority 4-5)', async () => {
      mockResolveTokenFull.mockResolvedValue(
        mockTokenResult('stored-token', 'keychain')
      );
  ...
  describe('Stored Credentials Fallback (Priority 4-5)', () => {
    it('should use keychain token when env is not available', async () => {
      mockResolveTokenFull.mockResolvedValue(
        mockTokenResult('keychain-token', 'keychain')
      );
    ...
    it('should use file token when keychain is not available', async () => {
      mockResolveTokenFull.mockResolvedValue(
  ...
  describe('Token Source Tracking', () => {
    ...
    it('should track source as octocode-storage when using stored credentials', async () => {
      // 'keychain' and 'file' sources map to 'octocode-storage'
      mockResolveTokenFull.mockResolvedValue(
        mockTokenResult('stored-token', 'keychain')
      );
  ...
  describe('Source Mapping', () => {
    it('should map keychain source to octocode-storage', async () => {
      mockResolveTokenFull.mockResolvedValue(
        mockTokenResult('token', 'keychain')
      );
  ...
  describe('Extended Source Mapping (Issue #9 - Legacy Sources)', () => {
    // Import the legacy API for testing source mapping
    let _setResolveTokenFn: typeof import('../src/serverConfig.js')._setResolveTokenFn;
    let _resetResolveTokenFn: typeof import('../src/serverConfig.js')._resetResolveTokenFn;

    beforeEach(async () => {
      const serverConfig = await import('../src/serverConfig.js');
      _setResolveTokenFn = serverConfig._setResolveTokenFn;
      _resetResolveTokenFn = serverConfig._resetResolveTokenFn;
    });
    ...
    it('should map legacy "env" source to "env:GITHUB_TOKEN" (default)', async () => {
      // Legacy tests use plain 'env' as source
      _setResolveTokenFn(async () => ({
      ...
      expect(getServerConfig().tokenSource).toBe('env:GITHUB_TOKEN');
    });

    it('should map legacy "octocode" source to "octocode-storage"', async () => {
      _setResolveTokenFn(async () => ({
        token: 'legacy-octocode-token',
        source: 'octocode',
      }));
    ...
    it('should map legacy "octocode-storage" source to "octocode-storage"', async () => {
      _setResolveTokenFn(async () => ({
    ...
    it('should preserve "env:*" prefixed sources from legacy resolver', async () => {
      _setResolveTokenFn(async () => ({
        token: 'legacy-token',
        source: 'env:GH_TOKEN',
      }));
      ...
      expect(getServerConfig().tokenSource).toBe('env:GH_TOKEN');
    });

    it('should preserve "gh-cli" source from legacy resolver', async () => {
      _setResolveTokenFn(async () => ({
        token: 'gh-cli-token',
        source: 'gh-cli',
      }));
    ...
    it('should handle legacy resolver returning null', async () => {
      _setResolveTokenFn(async () => null);
    ...
    it('should handle legacy resolver with undefined source', async () => {
      _setResolveTokenFn(async () => ({
        token: 'token-no-source',
    ...
    it('should handle legacy resolver errors gracefully', async () => {
      _setResolveTokenFn(async () => {
        throw new Error('Legacy resolver error');
      });
    ...
    it('should use getEnvTokenSource for plain "env" when specific env var is set', async () => {
      // When OCTOCODE_TOKEN is set, getEnvTokenSource returns 'env:OCTOCODE_TOKEN'
      process.env.OCTOCODE_TOKEN = 'octocode-env-token';

      _setResolveTokenFn(async () => ({
        token: process.env.OCTOCODE_TOKEN!,
        source: 'env',
      }));

      await initialize();

      // Should resolve to specific env var source
      expect(getServerConfig().tokenSource).toBe('env:OCTOCODE_TOKEN');
